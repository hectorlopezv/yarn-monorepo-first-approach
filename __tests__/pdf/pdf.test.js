...
import {PdfFile} from '@libprov/pdf/lib/pdf'
describe('testing suite', () => {
  ...
  it('testing unit Pdf', async () => {
    ...
    expect(screen.queryByText(/toolbar 1 de 1/i)).toBeNull()
    ...
    expect(
      ...
      document.querySelector('.react-pdf__Document').childElementCount,
    ).toBe(1)
    // eslint-disable-next-line testing-library/no-debug

    // await waitFor(
    //   () => expect(screen.queryByTestId('closeIcon_Pdf')).toBeInTheDocument(),
    //   {timeout: 5000},
    // )
    // debug()
    // user.click(screen.queryByTestId('closeIcon_Pdf'))
    // expect(setToogleShowd).toHaveBeenCalledTimes(1)
  })
})
