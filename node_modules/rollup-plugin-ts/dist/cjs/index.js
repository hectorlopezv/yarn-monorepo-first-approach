...
function visitExportDeclaration$3(options) {
    ...
    // Instead, we must declare the namespace inline and add an ExportDeclaration with a named export for it
        ...
        options.prependNodes(...importDeclarations.map(importDeclaration => preserveParents(importDeclaration, options)), preserveParents(factory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, factory, typescript), factory.createIdentifier(contResult.exportClause.name.text), moduleBlock, typescript.NodeFlags.Namespace), options), preserveParents(factory.createExportDeclaration(undefined, undefined, false, factory.createNamedExports([factory.createExportSpecifier(undefined, factory.createIdentifier(contResult.exportClause.name.text))]), undefined), options));
    }
...
function visitImportTypeNode$1(options) {
    ...
    if (contResult.qualifier == null) {
        ...
        options.prependNodes(...importDeclarations.map(importDeclaration => preserveParents(importDeclaration, options)), preserveParents(factory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, factory, typescript), factory.createIdentifier(namespaceName), moduleBlock, typescript.NodeFlags.Namespace), options));
...
function visitNamespaceImport$1(options) {
    ...
    // Otherwise, prepend the nodes for the SourceFile in a namespace declaration
    options.prependNodes(...importDeclarations.map(importDeclaration => preserveParents(importDeclaration, options)), preserveParents(factory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, factory, typescript), factory.createIdentifier(contResult.name.text), moduleBlock, typescript.NodeFlags.Namespace), options));
        ...
        async transform(code, file) {
            ...
            if (sourceDescription == null) {
                ...
                // Otherwise, pass it on to Babel to perform the rest of the transpilation steps
                    ...
                    const transpilationResult = await core.transformAsync(sourceDescription.code, Object.assign(Object.assign({}, babelConfigResult.config), { filenameRelative: ensureRelative(cwd, file), inputSourceMap: typeof sourceDescription.map === "string" ? JSON.parse(sourceDescription.map) : sourceDescription.map }));
                    if (transpilationResult == null || transpilationResult.code == null) {
                        return sourceDescription;
                    }
                    // Return the results
